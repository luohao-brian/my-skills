...
python3 {baseDir}/scripts/preset_shape_svg.py render-batch --input -   # JSON array of the same fields for several already-selected objects
python3 {baseDir}/scripts/preset_shape_svg.py describe chevron --compact  # objective identity/adjustment/connector/path facts, only for a serious candidate
