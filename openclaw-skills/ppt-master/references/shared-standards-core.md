...
- **Canvas authority**: New authoring writes `viewBox="0 0 W H"` with positive
  ...
  their own contracts.
- **Font portability and fidelity**: explicit SVG families are preserved as the
  concrete PowerPoint faces; they are not silently rewritten to another OS's
  counterpart. Every family used by the deck must therefore resolve on the
  declared target host through `font_preflight.py`. Generic CSS tails do not
  provide PowerPoint fallback, and legacy Unicode coverage faces are not valid
  generated title/body defaults. `@font-face` remains forbidden; the typography
  contract lives in [`strategist.md §g`](strategist.md).
- **Icon placeholders**: `<use data-icon="library/name">` is a pipeline-specific
  ...
  [`../templates/icons/_index.md`](../templates/icons/_index.md).
