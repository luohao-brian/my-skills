...
Everything here is a plan-only or Reference decision under [`ownership.md`](https://github.com/hugohe3/ppt-master/blob/v6.4.0/docs/rules/ownership.md): content, relationships, roster, rhythm, identity anchors, and resources bind; macro composition, motif, and pattern suggestions are References the Executor adjusts freely.
...
One primary stylistic library per pool, chosen from the four characters in the [icon README](../templates/icons/_index.md) table, with `stroke_width` from `{1.5, 2, 3}` for a stroke library; `simple-icons` is prepared from content for real brand marks. The pool is curated for broad semantic fit and synced before authoring; which icon a page uses is realization, never a preassignment.
