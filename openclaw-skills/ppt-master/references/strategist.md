...
🚧 **GATE — whole-document authoring**: Generate Step 4 reads `{baseDir}/templates/design_spec_reference.md`, authors the complete Design Spec once, passes Gate 1, then reads `{baseDir}/templates/spec_lock_reference.md` and authors the complete lock once. Do not scaffold or patch placeholders. Run `project_manager.py validate`; machine schemas, not remembered headings, own grammar validation.
...
See [`../templates/icons/_index.md`](../templates/icons/_index.md) for the current library inventory, counts, prefixes, and SVG placeholder details.
...
> 4. Put known basenames in the final batch. For an uncertain one, search the chosen style library — or `simple-icons` for a real brand mark — with `rg --files "{baseDir}/templates/icons/<library>" -g '*<keyword>*.svg'`; do not enumerate broad keyword families.
> 5. **Copy and validate in one batch** — run `python3 {baseDir}/scripts/icon_sync.py <project_path> <lib/name> [<lib/name> …]`. This both validates and materializes `<project>/icons/<lib>/`; skip per-file prechecks.
...
- Before accepting a family, run `python3 {baseDir}/scripts/font_preflight.py --family "<title-family>" --family "<body-family>" --require-emphasis --json`, adding every distinct Latin/EA face used by recurring roles. This uses `atsutil` on macOS before fontconfig, because `fc-match` cannot see some PowerPoint-usable reserved system families such as PingFang. An unavailable family, silent substitute, or missing real emphasis face is a failed recommendation; choose another installed family before writing the Design Spec.
- Legacy Unicode coverage faces such as `Arial Unicode MS` and `Lucida Sans Unicode` are not generated-deck title/body fonts. Broad glyph coverage does not make them a contemporary CJK text family; their limited native weights and weak screen stroke character create visibly softer PowerPoint output. Use them only after an explicit user requirement and record that exception for final-render review.
...
- Put an explicit installed EA family in every recurring stack that can render CJK, including annotation/code stacks containing mixed Latin+CJK text. A generic `sans-serif` / `monospace` tail is browser-only and cannot supply deterministic PowerPoint CJK fallback.
...
**Current inventory**: If `images/` is non-empty, run `python3 {baseDir}/scripts/analyze_images.py <project_path>/images` and read `analysis/image_analysis.csv` before recommending a source. Re-run after that folder changes.
...
```bash
python3 {baseDir}/scripts/chart_recall.py recall \
  --page P03 \
  --tag "time series" \
  --tag "three metrics" \
...
```bash
python3 {baseDir}/scripts/chart_recall.py validate <key> [<key> ...]
```
...
1. With Generate Step 4's retained complete final-confirmation state, read `{baseDir}/templates/design_spec_reference.md`.
...
5. Read `{baseDir}/templates/spec_lock_reference.md`; create the lock once or resynchronize stale derived state from the approved Design Spec and context. Retain identity/refinements and stable roles/routing; omit unnamed page-local values, do not reopen evidence, and make no new recommendation.
