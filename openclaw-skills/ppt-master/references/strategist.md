...
See [`../references/icon-library.md`](../references/icon-library.md) for the current library inventory, counts, prefixes, and SVG placeholder details.
...
> 4. Search for each concept's filename in the chosen library: `ls {baseDir}/templates/icons/<chosen-library>/ | grep <keyword>`
...
> 6. **Copy each chosen icon into the project as you confirm it** — `python3 {baseDir}/scripts/icon_sync.py <project_path> <lib/name> [<lib/name> …]`. This populates `<project>/icons/<lib>/` (the set the Executor embeds from) and, more importantly, **validates existence on the spot**.
...
**When selection includes B**, you must run `python3 {baseDir}/scripts/analyze_images.py <project_path>/images` before outputting the spec, and integrate scan results into the image resource list.
...
```bash
python3 {baseDir}/scripts/project_manager.py init <project_name> --format <canvas_format>
```
