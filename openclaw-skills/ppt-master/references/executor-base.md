...
Strategist chooses the library and inventory; Executor only implements. Library details and one-library rule: [`../references/icon-library.md`](../references/icon-library.md). This section defines placeholder syntax.
...
```bash
ls {baseDir}/templates/icons/chunk-filled/ | grep home
ls {baseDir}/templates/icons/tabler-filled/ | grep home
ls {baseDir}/templates/icons/tabler-outline/ | grep chart
ls {baseDir}/templates/icons/phosphor-duotone/ | grep house
ls {baseDir}/templates/icons/simple-icons/ | grep github
```
...
**Pure spoken narration**: notes are sent verbatim to the runtime TTS tool. Write only what should be spoken. No visible markers, no labeled meta-lines, no enumerated key-point lists, no duration annotations — anything you write outside the heading will be vocalized.
