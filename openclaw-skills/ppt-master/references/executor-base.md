...
```bash
python3 {baseDir}/scripts/project_manager.py page-context <project_path> P<NN> [--record-usage]
```
...
- Recurring normal-weight prose using `body_family` must stay in the `body ±2px` band. When intentionally using a smaller declared auxiliary role such as `footnote`, `source_url`, or `annotation`, put `data-pptx-text-role="<role>"` on that `<text>`/`<tspan>`; the value must name a numeric typography anchor and the effective size must remain in that role's band. Do not use an auxiliary anchor to disguise body copy.
...
- **Single-context ownership**: one execution context owns SVG generation for the whole deck; do not split pages across independent workers because they share upstream context and visual continuity
...
Strategist chooses at most one primary bundled stylistic library and may select `simple-icons` alone or alongside it; Executor implements from the complete prepared project-local pool. Library details and selection rules: [`../templates/icons/_index.md`](../templates/icons/_index.md). This section defines placeholder syntax.
...
**Execution gate**: run `python3 {baseDir}/scripts/font_preflight.py --family
"<face>" ... --require-emphasis --json` for every distinct locked Latin/EA
face before authoring pages. An exact family match is required; silent
substitution, a legacy Unicode coverage face, or a synthesized title/body bold
weight routes back to Strategist for a new installed face. Do not rely on a
browser rendering a later CSS fallback. Every stack used by mixed CJK text must
name its installed EA face explicitly.
