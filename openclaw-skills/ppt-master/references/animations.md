...
```bash
python3 {baseDir}/scripts/animation_config.py list-groups <project>   # real anchors after regrouping
python3 {baseDir}/scripts/animation_config.py scaffold <project>      # neutral editable scaffold (effect: none, {} placeholders)
python3 {baseDir}/scripts/animation_config.py validate <project>      # before export
python3 {baseDir}/scripts/svg_to_pptx.py <project>                    # reads <project>/animations.json automatically
```
...
```bash
python3 {baseDir}/scripts/sound_sync.py list --query <term>                       # optional exact filtering after the vocabulary is in context
python3 {baseDir}/scripts/sound_sync.py <project_path> <namespace>/<sound_id> [...]  # materialize only the chosen ids
```
