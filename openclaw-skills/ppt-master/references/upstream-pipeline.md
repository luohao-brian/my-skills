> Upstream pipeline baseline. In OpenClaw, `{baseDir}/SKILL.md` and
> `references/openclaw-runtime.md` override relative repository paths, output
> placement, dependency setup, and release-gate behavior in this document.
...
| Script | Purpose |
|--------|---------|
| `{baseDir}/scripts/source_to_md.py` | Unified source-to-Markdown dispatcher — default Step 1 entry for explicit file(s) or URL(s) |
| `{baseDir}/scripts/pptx_intake.py` | Standard PPTX intake enrichment — canvas / identity / slide geometry / tables / native chart data / SmartArt structure |
| `{baseDir}/scripts/project_manager.py` | Project init / validate / manage |
...
| `{baseDir}/scripts/preset_shape_svg.py` | Print one compact registry-backed native PowerPoint preset `<g>` to stdout for page or template insertion |
| `{baseDir}/scripts/total_md_split.py` | Speaker notes splitting |
| `{baseDir}/scripts/finalize_svg.py` | SVG post-processing (unified entry) |
| `{baseDir}/scripts/svg_to_pptx.py` | Export to PPTX |
| `{baseDir}/scripts/native_enhance_pptx.py` | Existing PPTX enhancement project init / validation / direct OOXML patch export |
| `{baseDir}/scripts/native_narration_pptx.py` | Backward-compatible entrypoint for existing PPTX notes / narration enhancement |
| `{baseDir}/scripts/update_spec.py` | Propagate a `spec_lock.md` color / font_family change across all generated SVGs |

For complete tool documentation, see `{baseDir}/references/script-catalog.md`.
...
| Index | Path | Purpose |
|-------|------|---------|
| Layout templates | `{baseDir}/templates/layouts/layouts_index.json` | Query available page layout templates |
| Brand presets | `{baseDir}/templates/brands/brands_index.json` | Query available brand identity presets (color / typography / logo / voice) |
| Visualization templates | `{baseDir}/templates/charts/charts_index.json` | Query available visualization SVG templates (charts, infographics, diagrams, frameworks) |
| Icon library | `{baseDir}/templates/icons/` | See `{baseDir}/references/icon-library.md`; search icons on demand with `ls templates/icons/<library>/ \| grep <keyword>` |
...
```bash
python3 {baseDir}/scripts/project_manager.py init <project_name> --format <format>
```
...
| Situation | Action |
|-----------|--------|
| Has source files (PDF/MD/etc.) | `python3 {baseDir}/scripts/project_manager.py import-sources <project_path> <source_files_or_dirs...> --move` |
...
The architecture has three independent reference bundles. Full schema in [`docs/zh/templates-architecture.md`](upstream-docs/templates-architecture.md). Summary:
...
2. **Launch + wait for Stage 1.** Background launch; the parent returns when the page writes the stage-1 `result.json`. **Long tool timeout — 600000 ms** (the `--wait` ≈590 s budget):
   ```bash
   python3 {baseDir}/scripts/confirm_ui/server.py <project_path> --daemon --wait
   ```
...
```bash
python3 {baseDir}/scripts/svg_quality_checker.py <project_path>/svg_output/<first_page>.svg
```
...
- Every static or rendered-geometry `warning` requires an explicit disposition: fix it, confirm the overlap/whitespace is intentional, or record it in the release note. Warnings do not fail the command by default, but they may not be silently ignored.
...
- The browser audit is mandatory for generated decks. It checks element and text collisions, canvas/container overflow, CSS contract drift, multi-viewport fit, edge crowding, and incomplete canvas use.
...
- [x] visual_layout_audit.py passed (0 errors; warnings dispositioned)
...
> **Judgement review (optional)**: deterministic browser geometry inspection already ran above. If the user asks for a deeper visual re-pass, run [`visual-review`](../workflows/visual-review.md) for contrast, hierarchy, narrative emphasis, and image meaning before Step 7.
...
**Step 7.4** — Audit the exported native package:
```bash
python3 {baseDir}/scripts/pptx_layout_audit.py <project_path>/exports/<exported_deck>.pptx
```
Fix any blocking geometry error in `svg_output/`, repeat the Step 6 release
gate, and then repeat Steps 7.2–7.4. Record the disposition of utilization or
edge-crowding warnings in the delivery note.
...
| Icon library | `references/icon-library.md` |
