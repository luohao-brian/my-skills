...
[View Design Spec Reference](../templates/design_spec_reference.md)
...
- **Slim lookup (discovery only)**: [layouts/layouts_index.json](../templates/layouts/layouts_index.json) — used to answer "what templates exist?". Step 3 triggers on an explicit directory path supplied by the user, not on names from this index.
...
- **Discovery index (no trigger)**: [brands/brands_index.json](../templates/brands/brands_index.json) — answers "what brands exist?"; Step 3 still requires an explicit directory path from the user
...
- **Library index (single source of truth)**: [charts/charts_index.json](../templates/charts/charts_index.json)
- **Style guide**: [CHART_STYLE_GUIDE.md](../templates/charts/CHART_STYLE_GUIDE.md)
...
- **Usage & style rules**: [icon-library.md](icon-library.md)
- **Search icons**: `ls {baseDir}/templates/icons/<library>/ | grep <keyword>`
