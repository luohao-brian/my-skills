...
### Step 4 — Write the manifest and call the image tool

Write `project/images/image_prompts.json` per §6. Validate it with `image_manifest.py`, then follow §7. Every `ai` row is generated by the image-generation tool available in the current runtime. The skill has no provider backend and no manual generation path.
...
**Sheet prompt convention** (one manifest item, `page_role: local`, `text_policy: none`):
...
For traceability, add optional `slice_grid` and `slice_names` fields to the sheet item in `image_prompts.json` after choosing the geometry. `image_manifest.py` preserves these fields, so they document the exact command that must be used for slicing.
...
```bash
python3 {baseDir}/scripts/slice_images.py <project>/images/illus_sheet.png --grid 2x3 \
    --names team,product,customer,growth,risk,vision --trim --alpha
```

**Three constraints that decide whether it looks good**:

1. **Flat background, matched to the slide.** Do not assume the image tool can return transparency. A flat sheet background (= deck background HEX) is what `--alpha` keys out and what makes non-keyed pieces blend.
...
3. **Check native output size before slicing.** Each cell is a fraction of the sheet. After generation, inspect the returned pixels and keep each sliced cell at least **1.5-2x** the intended display size. If the returned image is too small, regenerate a simpler or less crowded sheet instead of upscaling it.
...
| `items[].aspect_ratio` | yes | Container sizing | Exact layout ratio; `image_manifest.py pending` maps it to `landscape`, `square`, or `portrait` for the image tool |
...
| `items[].status` | yes | Script manages | `Pending` initially; `record` updates it to `Generated`, and `fail` updates it to `Failed` |
...
> Prerequisite: `images/image_prompts.json` exists. AI image generation requires an image-generation tool in the current runtime. There is no provider script and no manual generation path in this skill.

### Step 1 — Validate and list tasks
...
`pending` prints the exact prompt, output path, requested ratio, and the generic tool aspect: `landscape`, `square`, or `portrait`.

### Step 2 — Call the image tool

For every Pending or Failed task:

1. Call the image-generation tool listed by the current runtime.
2. Pass the task's `prompt` and `aspect_ratio` using the tool's declared parameter names.
3. Do not pass or select a provider, model, endpoint, API key, or quality preset.
4. If the tool accepts references, pass only references already recorded for that task.
5. Save the returned local file, or download the returned URL, to a temporary path outside the source repository.

If no image-generation tool is available, stop this step and report that AI image generation could not run. Do not install a provider SDK and do not switch to an external/manual path.
...
`record` copies the image to `project/images/<filename>`, verifies that it can be decoded, records its actual dimensions, and changes the task to `Generated`.

When a tool call fails:

```bash
python3 {baseDir}/scripts/image_manifest.py fail \
  project/images/image_prompts.json <filename> --error '<tool error>'
```

Stop before Executor while any task is Pending or Failed. Fix the prompt or runtime capability, then call the same image tool again.
...
`verify` succeeds only when every manifest item is `Generated` and every output is a non-empty, decodable image. Write the actual dimensions produced by the tool back into the resource list before SVG execution. Never upscale a returned image to imitate a requested resolution.
...
2. Call the current runtime's image-generation tool again for that item, then run `image_manifest.py record`
