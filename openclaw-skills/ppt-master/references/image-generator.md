...
- First `.env` found in this order: current working directory, skill directory (e.g. `~/.agents/{baseDir}/.env`), clone repo root, `~/.ppt-master/.env`
...
> Use provider-specific names only (e.g. `GEMINI_API_KEY`, `OPENAI_API_KEY`). See `.env.example` in clone mode or `{baseDir}/.env.example` in skill-install mode for the full set per backend.
...
- Still run `python3 {baseDir}/scripts/image_gen.py --render-md project/images/image_prompts.json` so the human-readable sidecar exists without touching any backend.
