...
| Project-canonical SVG syntax, compatible forms, and mapping boundary | [`references/shared-standards.md`](../../references/shared-standards.md) |
...
`<template_workspace>` is either `{baseDir}/templates/<kind>/<id>/` or `projects/<name>/`. Step 3 receives that root. The workspace is portable between locations without reshaping; global index registration is the only scope-specific behavior. Empty optional directories are absent, and template application never copies `exports/`.
...
**Runtime-owned generation.** AI images are generated only through the image-generation tool exposed by the current runtime. PPT Master owns prompts, output paths, manifest state, and validation; it does not own provider, model, endpoint, or credential selection.
...
**Manifest-first acquisition.** In-pipeline AI generation always writes `images/image_prompts.json`, even for one image. `image_manifest.py` validates the tasks and records image-tool results. Web acquisition separately uses `images/image_queries.json` for multi-row batches and `image_sources.json` for attribution/source tracking.
...
**Terminal status before Executor.** AI and slice rows must end in `Generated`; web rows may end in `Sourced` or `Needs-Manual`. `Pending` and `Failed` are not allowed to leak into Executor.
...
This is the key insight that's easy to miss when reading the code. Cleanup modules under `{baseDir}/scripts/svg_finalize/`, together with the local-reference expander, are used in two places for two different products.
