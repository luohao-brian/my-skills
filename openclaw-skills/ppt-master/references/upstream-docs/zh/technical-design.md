...
| 项目规范化 SVG 的语法、兼容形式与映射边界 | [`references/shared-standards.md`](../../../references/shared-standards.md) |
...
这张表只展示概念对应关系，不是对整个 SVG 标准的承诺，也不承诺所有映射语义无损。每项受支持能力都必须在 [`shared-standards.md`](../../../references/shared-standards.md) 中拥有明确的映射记录，说明项目规范写法、允许的兼容输入、目标 DrawingML 表达、保真度和拒绝条件；涉及 PPTX 回导的能力还要说明来源 PPTX / OOXML 语义。映射状态可以是精确、确定性归一化、显式 fallback、sidecar 或 unsupported；讲稿、动画、relationships 等 package 语义不必强行塞进 SVG，但必须明确由哪条路线承载。
...
`<template_workspace>` 可以是 `{baseDir}/templates/<kind>/<id>/`，也可以是 `projects/<name>/`。Step 3 接收这个根目录。工作区可在两个位置之间迁移而不改形；唯一的范围差异是全局索引注册。空的可选目录不创建，`exports/` 也不会复制进新项目。
...
**生成能力归运行时所有。** AI 图片只通过当前运行时提供的图片生成工具生成。PPT Master 负责提示词、输出路径、manifest 状态和结果校验；不负责选择 provider、模型、接口地址或密钥。
...
**Manifest-first 获取。** 流水线内的 AI 图片生成永远先写 `images/image_prompts.json`，哪怕只有一张图。`image_manifest.py` 校验任务并记录图片工具返回的结果。网络图片获取另用 `images/image_queries.json` 批量执行，并用 `image_sources.json` 追踪来源和致谢信息。
...
**Executor 前必须进入终态。** AI 和 slice 行必须是 `Generated`；web 行可以是 `Sourced` 或 `Needs-Manual`。`Pending` 和 `Failed` 不能漏进 Executor。
...
**为什么真正的硬约束留在上游。** 跨切的 SVG 创作与 PPTX 兼容性例外独家住在 [`shared-standards.md`](../../../references/shared-standards.md)。版式词表只指向这份合同，不再复述——这样规则变化时只有一个文件要改，词表里也不会留下一份过期副本继续暗中强制旧规则。

---

## 项目规范化 SVG 与兼容性边界

PowerPoint 的 DrawingML 是 SVG 表达力的严格子集，因此主编译路径不把“浏览器可以渲染”视为“项目可以导出”。只有在 [`references/shared-standards.md`](../../../references/shared-standards.md) 中登记了项目规范表达或显式兼容形式，并且拥有确定 DrawingML 映射的词汇，才属于可接受输入。该文件是语法、结构、单位、metadata、兼容别名、保真度和拒绝条件的唯一权威；本架构文档只定义分层原则，不复制具体规则。
...
| `svg_final/` | 支持直接打开 SVG 的编辑器内即时预览、浏览器单页预览、手动作为 SVG 图片插入 | `.pptx` 通常不能在代码编辑器中直接预览；`svg_output/` 因图标 / 图片是外部引用，直接渲染可能不完整。PowerPoint 手工“转换为形状”不在支持范围 |
...
这是读代码时容易忽略的关键事实。`{baseDir}/scripts/svg_finalize/` 下的清理模块和本地引用展开器，会在两个地方被使用，服务两份不同的产物。
