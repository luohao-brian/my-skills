...
```bash
# 单文件 SVG 合同
python3 {baseDir}/scripts/svg_quality_checker.py \
  {baseDir}/templates/charts/<key>.svg

# Catalog key
python3 {baseDir}/scripts/chart_recall.py validate <key>

# 可安全压缩的页面坐标（默认 dry-run）
python3 {baseDir}/scripts/compact_svg_coordinates.py \
  {baseDir}/templates/charts/<key>.svg
```
