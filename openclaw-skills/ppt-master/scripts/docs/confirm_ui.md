...
```bash
python3 {baseDir}/scripts/confirm_ui/server.py <project_path> --daemon --wait   # launch + wait for Stage 1
python3 {baseDir}/scripts/confirm_ui/server.py <project_path> --wait-only --wait-stage stage2  # Stage 2: wait for the design-system handoff
python3 {baseDir}/scripts/confirm_ui/server.py <project_path> --wait-only       # Stage 3: wait for the final result
python3 {baseDir}/scripts/confirm_ui/server.py <project_path> --daemon
python3 {baseDir}/scripts/confirm_ui/server.py <project_path> --daemon --port 5051
python3 {baseDir}/scripts/confirm_ui/server.py <project_path> --no-browser
python3 {baseDir}/scripts/confirm_ui/server.py <project_path> --timeout 0   # disable idle auto-shutdown
python3 {baseDir}/scripts/confirm_ui/server.py <project_path> --shutdown    # Step 4 cleanup (idempotent)
```
...
- `image_strategy` is omitted from `result.json` unless `image_usage` includes `ai`. The chosen strategy candidate is locked verbatim by Strategist h.5; generated images always use the current runtime's image-generation tool.
