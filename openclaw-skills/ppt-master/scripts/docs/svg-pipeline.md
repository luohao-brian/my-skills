...
python3 {baseDir}/scripts/svg_to_pptx.py <project_path> --no-merge   # strict line-fidelity mode (see below)
...
- Recorded narration is opt-in:
  - `audio_manifest.py prepare` creates one task per slide; the agent sends each task to the current runtime's TTS tool
  - Narration text is read strictly from the matching `notes/*.md` file; preparation only skips Markdown heading lines (`# ...`) and does not summarize or rewrite the notes
  - `--recorded-narration audio` prepares PowerPoint's "recorded timings and narrations": every slide must have matching `m4a` / `mp3` / `wav` audio, its duration must be readable by `ffprobe`, WAV metadata, or `afinfo`, and `--animation-trigger on-click` is rejected
  ...
  - Provider, model, voice, and rate are runtime TTS configuration and are not selected by this skill
