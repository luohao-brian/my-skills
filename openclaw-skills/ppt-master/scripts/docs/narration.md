...
Tool behavior behind the [`generate-audio`](../../workflows/stages/generate-audio.md) stage: `notes_to_audio.py`, `narration_sync.py`, the narrated `svg_to_pptx.py` export, `powerpoint_video.py`, `video_sound_mix.py`, and `video_subtitles.py`. The stage owns when each runs and what the user confirms; this page owns what the tools do. Model and audio-parameter recommendations live in [`docs/audio-narration.md`](https://github.com/hugohe3/ppt-master/blob/v6.6.0/docs/audio-narration.md).
...
```bash
python3 {baseDir}/scripts/notes_to_audio.py --list-voices --locale <locale>              # edge
python3 {baseDir}/scripts/notes_to_audio.py --provider <elevenlabs|minimax|qwen|cosyvoice> --list-voices
python3 {baseDir}/scripts/notes_to_audio.py <project_path> --voice <ShortName> --rate=<rate> [--pitch=<pitch>] [--volume=<volume>] [--concurrency <N>]
python3 {baseDir}/scripts/notes_to_audio.py <project_path> --provider elevenlabs --voice-id <id> --elevenlabs-model eleven_multilingual_v2
python3 {baseDir}/scripts/notes_to_audio.py <project_path> --provider minimax --voice-id <id> --minimax-model speech-2.8-hd
python3 {baseDir}/scripts/notes_to_audio.py <project_path> --provider qwen --voice-id <voice> --qwen-model qwen3-tts-flash --qwen-language-type Chinese
python3 {baseDir}/scripts/notes_to_audio.py <project_path> --provider cosyvoice --voice-id <voice> --cosyvoice-model cosyvoice-v3-flash [--cosyvoice-audio-only]
```
...
```bash
python3 {baseDir}/scripts/narration_sync.py fingerprint <project_path>
python3 {baseDir}/scripts/narration_sync.py animations <project_path> --narration-start-floor 0.8 --narration-padding 0.5 --force
python3 {baseDir}/scripts/narration_sync.py subtitles <project_path> --pptx <final_narrated_pptx> --force
```
...
```bash
python3 {baseDir}/scripts/svg_to_pptx.py <project_path> --recorded-narration audio --narration-start-floor 0.8 --narration-padding 0.5 --inherit-motion-from "<base_postflight_report>"
...
python3 {baseDir}/scripts/powerpoint_video.py <final_narrated_pptx> -o <raw_powerpoint_video.mp4>
```
...
```bash
python3 {baseDir}/scripts/video_sound_mix.py <project_path> --pptx <final_narrated_pptx> --trace <final_narrated_trace> --video <raw_powerpoint_video.mp4> -o <final_mixed_video.mp4> --stem-output <final_sfx_stem.wav> --report-output <sound_mix_report.json> --force
```
...
```bash
python3 {baseDir}/scripts/video_subtitles.py <project_path> --video <final_delivery_video.mp4> --language <language> --force
```
