...
```bash
python3 {baseDir}/scripts/pptx_template_import.py "<reference_template.pptx>" [--inheritance-mode layered|both|flat]
```
...
```bash
python3 {baseDir}/scripts/pptx_template_import.py "<source.pptx>" -o "<import_workspace>" --inheritance-mode both
python3 {baseDir}/scripts/mirror_template_materialize.py "<import_workspace>" "<template_workspace>" [--kind deck|layout]
python3 {baseDir}/scripts/svg_quality_checker.py "<template_workspace>/templates" --template-mode --canonical-authoring
python3 {baseDir}/scripts/template_preview_pptx.py "<template_workspace>" -o "<preview.pptx>"
```
...
`{baseDir}/templates/<kind_dir>/` uses `templates/design_spec.md`;
...
```bash
python3 {baseDir}/scripts/svg_quality_checker.py "<template_source>" --template-mode --canonical-authoring [--format <canvas_format>]
```
...
```bash
python3 {baseDir}/scripts/template_preview_pptx.py "<authoring_workspace>" [--force] [--native-charts-and-tables -o <distinct_path>]
```
...
```bash
python3 {baseDir}/scripts/apply_template.py <project_path> --root <workspace_root> [--root <workspace_root> ...] [--dry-run] [--skip-validation]
```
...
```bash
python3 {baseDir}/scripts/register_template.py <template_id> --kind brand|style|layout|deck [--dry-run]
python3 {baseDir}/scripts/register_template.py --kind style|deck|layout --rebuild-all
```
