...
python3 {baseDir}/scripts/project_manager.py import-sources "<project_dir>" "<source.pptx>" "<material...>"
```
...
```bash
python3 {baseDir}/scripts/template_fill_pptx.py analyze "<project_dir>/sources/<source.pptx>" -o "<project_dir>/analysis/<stem>.slide_library.json"
```
...
```bash
python3 {baseDir}/scripts/template_fill_pptx.py scaffold "<project_dir>/analysis/<stem>.slide_library.json" -o "<project_dir>/analysis/fill_plan.json" --slides "1,3,4"
```
...
Each `notes` value is **pure spoken narration**: write only what a presenter would say aloud, so the same text can later become one runtime TTS task. The note explains and connects; it must not just restate the words already on the slide.
...
```bash
python3 {baseDir}/scripts/template_fill_pptx.py check-plan "<project_dir>/analysis/<stem>.slide_library.json" "<project_dir>/analysis/fill_plan.json" -o "<project_dir>/analysis/check_report.json"
```
...
```bash
python3 {baseDir}/scripts/template_fill_pptx.py apply "<project_dir>/sources/<source.pptx>" "<project_dir>/analysis/fill_plan.json" -o "<project_dir>/exports/<output.pptx>"
```
...
```bash
python3 {baseDir}/scripts/template_fill_pptx.py validate "<project_dir>"
```
