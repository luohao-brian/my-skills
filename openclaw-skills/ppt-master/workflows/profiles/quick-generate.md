...
| PDF / DOCX / Office / XLSX / XLSM / PPTX / EPUB / HTML / LaTeX / RST / web URL | `python3 {baseDir}/scripts/source_to_md.py <file_or_URL_or_dir> [...]` (`-t <type>` only when detection is ambiguous; `-o` only for a required output path) |
...
```bash
python3 {baseDir}/scripts/project_manager.py init <project_name> --quick-generate --dir <absolute-projects-root>
...
| Bundled/custom/brand SVG icon | [Icon library contract](../../templates/icons/_index.md): one primary generic library per pool (`icon_sync.py` rejects mixed batches), synced without page assignment; `simple-icons` for named brands |
