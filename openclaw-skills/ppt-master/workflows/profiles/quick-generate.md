...
```bash
python3 ${SKILL_DIR}/scripts/project_manager.py init <project_name> \ --dir <absolute-projects-root>
...
| Bundled/custom icon | Follow the [icon library contract](../../templates/icons/_index.md), choose one coherent primary library, sync a useful project pool covering recurring semantics and likely page-local needs without assigning icons to pages, and choose from that prepared pool during SVG authoring |
