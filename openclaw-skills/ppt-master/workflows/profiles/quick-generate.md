...
```bash
python3 {baseDir}/scripts/project_manager.py init <project_name> \
  --format <format> --quick-generate --dir <absolute-projects-root>
```
...
| Bundled/custom icon | Follow the [icon library contract](../../templates/icons/_index.md), resolve the selected SVG under project `icons/`, and use `icon_sync.py` for bundled icons |
...
**Typography**: name installed concrete Latin/EA families in the SVG; do not
depend on a lock or generated font asset. Before authoring, run `python3
{baseDir}/scripts/font_preflight.py --family "<face>" ... --require-emphasis
--json`. Do not use a legacy Unicode coverage face such as `Arial Unicode MS`
...
Fix every blocking error and rerun the same command. Measure the rendered SVGs before export:

```bash
python3 {baseDir}/scripts/visual_layout_audit.py <project_path>
```

Objective browser-geometry errors block. Collision/utilization/edge findings are advisory and must be judged against the intended composition. Then export:
...
Run `pptx_delivery_check.py` and `pptx_layout_audit.py` against the published PPTX, then render and inspect a final montage. Package/objective geometry errors block; advisory overlap/utilization findings require visual judgement.
...
- [x] Browser SVG geometry, published PPTX package/geometry, and final montage were inspected
