...
| `generate-audio` | [`generate-audio.md`](./generate-audio.md) | User asks for recorded narration, voiceover, audio, or video-style export | Post-processing/export context exists; notes available; runtime TTS tool available | Do not call a provider API or install a TTS backend | After Step 7 | Audio files and optional narration-embedded PPTX | Stops when the TTS tool is unavailable or any page audio fails validation |
