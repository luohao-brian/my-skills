...
Spec naming and kind declaration follow [`templates/_index.md`](../../templates/_index.md); a root exposing several kind-qualified specs contributes all of them. Never accept only another project's inner `templates/` directory — that omits sibling assets.
...
Read [`templates/_index.md`](../../templates/_index.md), then only the README for each supplied kind:

| Kind | Schema | Owned segment |
|---|---|---|
| `brand` | [`templates/brands/_index.md`](../../templates/brands/_index.md) | Identity: color, typography, logo, voice/tone, icon style |
| `style` | [`templates/styles/_index.md`](../../templates/styles/_index.md) | Direction/method: communication method, visual language, composition, information-expression defaults |
| `layout` | [`templates/layouts/_index.md`](../../templates/layouts/_index.md) | Structure: canvas, page structure, semantic text roles, page types, SVG roster |
| `deck` | [`templates/decks/_index.md`](../../templates/decks/_index.md) | Application plus integrated identity and structure |
...
```bash
python3 {baseDir}/scripts/svg_quality_checker.py "<workspace_root>/templates" --template-mode --canonical-authoring
```
...
```markdown
> **Installed from**: `{baseDir}/templates/brands/mckinsey/` (library)
```
...
```bash
python3 {baseDir}/scripts/apply_template.py <project_path> --root <workspace_root> [--root <workspace_root> ...]
```
