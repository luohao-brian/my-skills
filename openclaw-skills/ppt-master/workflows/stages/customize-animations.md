...
Unless an all-motion disable bypasses it, validate an existing sidecar first: `python3 {baseDir}/scripts/animation_config.py validate <project_path>`.
...
When `animations.json` was created or changed after §1, run `python3 {baseDir}/scripts/animation_config.py validate <project_path>`, then return to the owning export path — Default [`generate-pptx.md`](../generate-pptx.md) Step 7.1; Quick [`quick-generate.md`](../profiles/quick-generate.md) §4 — both of which read the sidecar automatically. If §2 changed `svg_output/`, complete the owning route's final SVG rerun before returning; never finalize or export from this stage.
