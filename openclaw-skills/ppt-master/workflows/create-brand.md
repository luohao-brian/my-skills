...
> Standalone preset-creation workflow. Output is a complete brand workspace at `{baseDir}/templates/brands/<brand_id>/`. A brand keeps the common workspace shape but omits the SVG page roster: Strategist locks identity while Executor designs pages freely.
...
| Brand site URL | `python3 {baseDir}/scripts/source_to_md/web_to_md.py <URL>`, then `Read` the result | color references, font references, voice/tone |
| Branded PPTX | `python3 {baseDir}/scripts/source_to_md/ppt_to_md.py <file>`, then read theme XML | colors, typography (literal) |
| Brand PDF | `python3 {baseDir}/scripts/source_to_md/pdf_to_md.py <file>` | voice/tone; sometimes color/font references |
...
```bash
mkdir -p "{baseDir}/templates/brands/<brand_id>/templates"
```
...
```bash
python3 {baseDir}/scripts/register_template.py --kind brand <brand_id> --dry-run
```

Fix every reported error, then register the same workspace:

```bash
python3 {baseDir}/scripts/register_template.py --kind brand <brand_id>
```
...
```markdown
## ✅ Brand Saved
- Path: `{baseDir}/templates/brands/<brand_id>/`
- Files: templates/design_spec.md{, images/logo.<ext>}{, images/*}{, icons/*}
- Fields locked: <list>
- Provenance: <fact / approx / user counts>

How to use in a project:
- Include the brand workspace path in the initial Step 3 input, for example: "Create a Q4 summary using {baseDir}/templates/brands/<brand_id>/"
...
- To edit: modify `templates/brands/<brand_id>/templates/design_spec.md`, then re-run `python3 {baseDir}/scripts/register_template.py --kind brand <brand_id>`
```
