...
description: 生成和迭代单文件 HTML 横向演示文稿，支持电子杂志风与瑞士国际主义风、演讲者视图、观众屏同步与讲稿备注，并提供版式契约、静态校验、视觉检查和安全组装脚本。当用户要制作网页 PPT、演讲 slides、杂志风或 Swiss Style deck，或要检查现有 guizang HTML deck 时使用。
...
来源：本 skill 基于歸藏维护的 [guizang-ppt-skill](https://github.com/op7418/guizang-ppt-skill)；上游基线为 `c91369c`。来源信息只用于识别，不要写入生成的 deck。
...
需要现场讲述、排练或观众屏时读 `references/presenter-mode.md`。处理图片或截图时按需读 `references/image-prompts.md` 和 `references/screenshot-framing.md`。交付前读 `references/checklist.md`。
...
2. 先列 `页码 → data-slide-id → 版式 → 选择原因 → 图片槽位 → data-animate → 讲述目的/时长/转场`。Style B 的正文页只能使用 `S01-S22`；实验版式必须得到用户明确许可。
3. 从对应 layouts 复制骨架。不要混用 Style A/B 类名，不要发明未在模板 CSS 中定义的类。每页写唯一、稳定、语义化的 `data-slide-id`。
4. 把所有 `<section>` 放入独立的 `slides.html`，把与页面顺序和 ID 一一对应的备注数组放入 `speaker-notes.json`，再用安全组装脚本生成最终 deck，避免误删模板底部运行时：
...
node {baseDir}/scripts/create-deck.mjs \
  ...
  --title "演示标题" \
  --slides ./slides.html \
  --notes ./speaker-notes.json \
  --output ./ppt/index.html
```

Style A 将 `--style swiss` 改成 `--style magazine`。

5. 检查两套模板的演讲 runtime 同步，并校验成品的页面 ID、备注和演讲者功能：

```bash
node {baseDir}/scripts/check-presenter-runtime-sync.mjs
node {baseDir}/scripts/validate-presenter-mode.mjs ./ppt/index.html
```
...
7. 运行四视口视觉审计。检查项：元素重叠、页面/容器越界、底部安全区、内容只挤在局部。CSS class 与 recipe 仍由上一步检查：
...
8. 在浏览器逐页检查标题层级、底部导航安全区、深色背景对比度、图片裁切、动画终态和 ESC 索引页。按 `P` 实测演讲者模式、观众屏恢复、备注保存、计时、排练、宫格、标注和黑白屏/冻结控制。

## Style B Contract

- 每页必须写 `data-layout`、`data-animate` 和 `data-slide-id`；版式与动效组合必须匹配 `references/swiss-contract.json`。
...
- 输出可直接打开的单 HTML deck，图片使用相对路径；页面 ID 与 `SPEAKER_NOTES` 必须一一对应。
- 替换所有 `[必填]` 占位符。
