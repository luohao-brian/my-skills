...
使用安全组装脚本时，把上面数组本身保存为合法 JSON，例如 `speaker-notes.json` 只包含 `[{...}]`，不要包含 `const SPEAKER_NOTES =` 或 `window.__SPEAKER_NOTES__`。然后通过 `create-deck.mjs --notes ./speaker-notes.json` 注入模板。
