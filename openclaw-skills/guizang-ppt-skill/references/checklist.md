...
**根因**:生成时把 Swiss 当成风格包,自由组合了新的 P23/P24/自绘 SVG 页面,没有从 `swiss-contract.json` 的 22 个稳定版式里选。
...
**现象**:最顶上的中文标题在页面中间,偏离登记版式的内容轴。

**做法**:
- 除 `S03/S09/S10` 这类 statement/split 版式外,顶部标题必须贴模板的左上内容轴。
- 不要把小标题放左列、大标题放右侧大列,这会导致标题视觉居中。
- 如果需要标题 + 说明两列,必须复制 `S11` 或 `S17` 的 golden 骨架,不要自写 `4fr 8fr`。
...
**根因**:`.canvas-card` 已经自带 `padding:5.6vh 5vw 8vh`。如果在主体区再写 `padding:5vh 5vw 4vh`,水平方向就变成 `5vw + 5vw = 10vw`,主体比 chrome-min 多内缩 5vw。

**做法**:
- 主体那层 `padding:0`,只用 grid `gap` 控垂直间距
- chrome-min 与主体之间的间距由 `.chrome-min{margin-bottom:48px}` 提供,**不要**在主体顶部叠 `margin-top` / `padding-top`
- split 模式例外:`.slide.split .canvas-card{padding:0}`,两个 `.half` 自己定 `padding:5.6vh 3.6vw 8vh`
...
**现象**:图片像普通 PPT 插图,圆角、阴影、比例混乱;多张截图高度不一,或生成式配图自带标题/页脚,和页面 chrome 重复。
...
**现象**:生成页看起来像瑞士风,但字重、间距、时间线或卡片密度逐渐偏离仓库基准。

**根因**:把新增图片版式或实验结构写成了全局样式修改,或无意改动了原始基座类,例如 `.h-hero` / `.h-xl` 字重、`.tl-node` 列宽、`.duo-compare` 间距。

**做法**:
- 仓库内 `assets/swiss-golden.html` 是 Swiss 主题的可复现 golden source；版式源块见 `templates/swiss-golden-slides.html`，机器契约见 `references/swiss-contract.json`
- 大标题使用轻字重；`.h-hero` / `.h-xl` / `.h-hero-zh` / `.h-xl-zh` 不要改成 800/900
- 修改模板 class、布局或 `RECIPES` 时，同步更新 `references/swiss-contract.json` 与 `templates/swiss-golden-slides.html`
- 新增图片能力必须绑定到 S22/S15/S16 登记槽位，不要发明新正文结构
...
**做法**:
- **chrome = 杂志页眉 / 导航标签**:跨多页可相同(如 "Act II · Workflow"、"Data · Result"、"publication.example · 2026.04")
...
- 通过图像生成或编辑能力制作的配图只是嵌入素材,不要让图片自带页眉、页脚、标题、页码、角标、署名或装饰边框
