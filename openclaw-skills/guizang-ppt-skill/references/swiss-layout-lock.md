...
- `assets/swiss-golden.html`：完整 22 页可运行 golden deck。
- `templates/swiss-golden-slides.html`：可复制的 22 个 slide 源块。
- `references/swiss-contract.json`：版式、recipe 与必需 class 的机器契约。

不要依赖作者机器上的绝对路径。修改模板、版式或 recipe 后，必须重新生成 golden deck 并运行 contract、静态和视觉检查。
...
- 如果原始截图比例不可控,先按 `references/screenshot-framing.md` 做程序化比例适配;只有长截图、极窄截图或信息需要重构时,才使用当前可用的图像生成或编辑能力制作“截图再设计”。
