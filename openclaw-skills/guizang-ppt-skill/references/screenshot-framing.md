...
用于把用户提供的产品截图、网页截图、代码截图和设计稿截图处理成符合模板比例的图片资产。默认采用保真截图适配：创建与目标槽位同宽高比的画布，铺设匹配主题的背景，将原截图等比缩放，并按统一的内边距和对齐方式放置。除非用户允许且信息确实需要重构，否则不得重绘截图内容。

## 优先级

1. **程序化适配优先**:截图内容、文字、UI 细节需要保真时,不要重画;创建目标比例画布,把原截图等比缩放后放入画布。
2. **生成式重构仅作例外**:只有原图过长、过窄、信息太乱、需要 UI 情景化或概念化表达时,才使用当前可用的图像生成或编辑能力制作“截图再设计 / UI 情景图”。
...
只问用户尚未给出的项目。没有特别说明时，默认保真展示、保留全部文字和数据、槽位跟随所选版式、对齐方式自动判断；截图中出现账号、头像或项目名且演示将对外发布时，再询问是否脱敏。
...
2. **再选处理方式**:
   ...
   - 原图不可用或需要解释概念:再使用当前可用的图像生成或编辑能力做生成式重构。
...
本 Skill 已经内置一组预生成背景。处理截图时**优先使用这些资产**,不要为每张截图实时重新生成背景。只有用户明确要求新风格、现有主题缺失,或背景与内容明显不匹配时,才使用当前可用的图像生成能力创建新背景。
...
| 概念解释用的 UI 情景图 | 可以使用当前可用的图像生成或编辑能力进行生成式重构 |
