...
6. Expect stdout JSON with `success`, `backend`, `images`, `local_path`, `remote_url`, `model`, `count`, `aspect_ratio`, `resolution`, `size`, `usage`, and `request_id`. A response with no downloaded local image is a failed call.
