...
metadata: {"openclaw":{"skillKey":"tts","emoji":"🔊","homepage":"https://github.com/luohao-brian/my-skills/tree/main/openclaw-skills/tts","requires":{"anyBins":["python3","python"],"env":["VOLC_AGENT_PLAN_API_KEY"]},"primaryEnv":"VOLC_AGENT_PLAN_API_KEY","install":[{"id":"python-deps","kind":"uv","package":"requests>=2.32,<3"}]}}
---
...
1. Require `VOLC_AGENT_PLAN_API_KEY`; the script also accepts `ARK_API_KEY` and `OPENAPI_API_KEY` as compatibility fallbacks.
2. Write the audio file to the requested output path, or to `./outputs/tts/` when no path is supplied.
3. Return JSON containing `success`, `local_path`, `format`, `bytes`, `speaker`, and request metadata.
4. If credentials, resource id, endpoint, or network access fail, report the exact missing variable or API error.

## Defaults

- Endpoint: Ark plan TTS endpoint from Hermes Ark plugin.
