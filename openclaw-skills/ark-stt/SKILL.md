---
name: ark-stt
description: 使用 Ark Agent Plan 流式语音识别接口把本地 16 kHz 音频转成文字。适用于录音转写、字幕初稿、会议音频识别、语音消息解析和 STT 接口验证。
...
1. The script reads only `ARK_AGENT_PLAN_API_KEY` and uses the fixed Agent Plan endpoint, `volc.seedasr.sauc.duration` resource, and new-console `X-Api-Request-Id` plus `X-Api-Sequence` handshake.
