...
本节只展示 `groups.media_customization`，最多 8 条。能力必须来自精确 HF tag 或最终入选模型的 model card 明确证据。

### 1. 模型名称

- 背景与用途：用一句话合并模型形态与核心能力，例如说明它是完整模型、量化主模型、LoRA、工作流还是组件包，以及主要输入输出；不要拆成“它是什么”“能做什么”两行。
- 部署信息：用一句话概括 ComfyUI 运行路径、应导入的 workflow、权重加载位置和关键组件关系；不要另起“怎么用”或“主要运行方式”。
...
- ComfyUI 条目使用“背景与用途 + 部署信息”的密度；组件关系已经在“部署信息”或“下载与组件”中说明时，不再重复生成“必需依赖”。
...
- `media_customization` 只按 `signals` 写能力。`source=hf-tag` 是结构化标签，`source=model-card` 是发布方自述；两者都不等同于实际工作流验证。
